...
**NOTE:** The function `log_pY_z()` in the source `R` code [`TESTsbm.R`](https://github.com/danieledurante/TESTsbm/blob/master/Data%20and%20Codes/TESTsbm.R) has been updated to correct a minor bug in the original implementation. Due to this modification, some values obtained in this [`Tutorial.md`](https://github.com/danieledurante/TESTsbm/blob/master/Tutorial.md) may not perfectly coincide with those reported in the simulation study illustrated in the article. However, the magnitude of these differences is minor and does not affect the final conclusions.
